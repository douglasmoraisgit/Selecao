...
import UsuarioModel from '../model/UsuarioModel.js';  // ✅ NOVO - Autenticação
...
import UsuarioView from '../view/UsuarioView.js';  // ✅ NOVO - Menu Usuário
...
import UsuarioController from './UsuarioController.js';  // ✅ NOVO - Autenticação
...
export default class AppController extends EventEmitter {
    ...
    async init() {
        ...
        try {
            ...
            const autenticado = await this.initUsuario();
            if (!autenticado) {
                console.warn('⚠️ Usuário não autenticado');
                // Continua mesmo sem autenticação, mas sem menu de usuário
            }
            ...
            // Log do usuário se autenticado
            if (autenticado && this.models.usuario) {
                console.log('✅ Sistema inicializado!');
                console.log('👤 Usuário logado:', this.models.usuario.getNome());
                console.log('🏪 Loja:', this.models.usuario.getLojaNome());
            } else {
                console.log('✅ Sistema inicializado!');
            }
    ...
    /**
     * Inicializa módulo de usuário (autenticação)
     */
    async initUsuario() {
        ...
        try {
            // Cria instâncias
            this.models.usuario = new UsuarioModel();
            this.views.usuario = new UsuarioView();
            this.controllers.usuario = new UsuarioController(
                this.models.usuario,
                this.views.usuario
            );
            
            // Escuta eventos
            this.controllers.usuario.on('usuarioCarregado', (dados) => {
                console.log('👤 Usuário autenticado:', dados.usuario.nome);
                if (dados.loja) {
                    console.log('🏪 Loja:', dados.loja.nome);
                }
            });
            
            this.controllers.usuario.on('logout', () => {
                console.log('🚪 Logout realizado');
                window.location.href = 'login.html';
            });
            
            // Inicializa (carrega dados do servidor)
            const sucesso = await this.controllers.usuario.init('userMenuContainer');
    ...
    getUsuarioLogado() {
        return this.models.usuario?.getUsuario();
    }
    
    getUsuarioId() {
        return this.models.usuario?.getId();
    }
    
    getUsuarioNome() {
        return this.models.usuario?.getNome();
    }
    
    getLoja() {
        return this.models.usuario?.getLoja();
    }
    
    getLojaId() {
        return this.models.usuario?.getLojaId();
    }
    
    getLojaNome() {
        return this.models.usuario?.getLojaNome();
    }
    
    getVendedor() {
        return this.models.usuario?.getNome() || 'Sistema';
    }
    
    isAdmin() {
        return this.models.usuario?.isAdmin();
    }
    
    isLogado() {
        return this.models.usuario?.isLogado();
    }
