 ...
 * - 'buscarComFiltroColoracao' → Buscar com filtro permite_colorir { permiteColorir }
 * 
 * @author OptoFreela
 */
...
export default class NavigationController extends EventEmitter {
    ...
    async renderCurrentLevel() {
        ...
        if (currentNode?.id === 'coloracao') {
            ...
            // NOVO: Se só sobrou "Sem Coloração", significa que a marca/família não permite colorir
            const apenasSeColoracao = children.length === 1 && 
                (children[0].id === 'cor-sem' || children[0].label === 'Sem Coloração');
            
            if (apenasSeColoracao) {
                console.log('🚫 Marca/Família não permite colorir - emitindo busca com filtro permite_colorir=Sim');
                // Emite evento para fazer busca especial que retornará 0 produtos
                this.emit('buscarComFiltroColoracao', { permiteColorir: 'Sim' });
            }
        }
