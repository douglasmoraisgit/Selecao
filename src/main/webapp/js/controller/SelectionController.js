    ...
    handleConfigSelection(item) {
        ...
        if (tipoConfig) {
            ...
            if (jaExiste) {
                ...
                // Inclui campos adicionais para coloração (tipoColoracao, corHex)
                ...
                // Adiciona campos específicos de coloração se existirem
                if (item.tipoColoracao) {
                    itemParaSalvar.tipoColoracao = item.tipoColoracao;
                }
                if (item.corHex) {
                    itemParaSalvar.corHex = item.corHex;
                }
                
                this.selecaoAtivaModel.adicionar(tipoConfig, itemParaSalvar);
                console.log(`✅ Adicionado [${tipoConfig}]:`, item.label, item.tipoColoracao ? `(${item.tipoColoracao})` : '');
