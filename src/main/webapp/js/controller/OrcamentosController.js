...
export default class OrcamentosController extends EventEmitter {
    ...
    bindViewEvents() {
        ...
        // Imprimir orçamento
        this.view.on('imprimir', ({ orcamento, id }) => {
            this.imprimirOrcamento(id);
        });
    ...
    /**
     * Abre página de impressão do orçamento
     */
    imprimirOrcamento(id) {
        ...
        const url = `${this.baseUrl}/ImprimirOrcamento?id=${id}`;
        
        // Abre em nova janela
        const printWindow = window.open(url, '_blank', 'width=900,height=700');
        
        if (!printWindow) {
            this.mostrarToast('❌ Popup bloqueado! Permita popups para imprimir.');
        }
    }
